...
# Can use BOUTS and PAUSES for each npz to do binned analysis...
# Can also do pause analysis
# Bouts and pauses all stored in each NPZ...nice one
...
Percent_Moving_NS_BINS_CTL = np.zeros(numFiles)
Percent_Moving_S_BINS_CTL = np.zeros(numFiles)
...
for f, filename in enumerate(npzFiles_ctl):
    ...
    # Bin bouts and pauses into one min bins
    # Compute mobility (percent time moving)...and possibly other metics
    Percent_Moving_NS_BINS = []
    Percent_Moving_S_BINS = []

    # Make an array with all summary stats
    Percent_Moving_NS_BINS_CTL[f] = Percent_Moving_NS_BINS
    Percent_Moving_S_BINS_CTL[f] = Percent_Moving_S_BINS
...
# Allocate space for summary data
Percent_Moving_NS_BINS_ISO = np.zeros(numFiles)
Percent_Moving_S_BINS_ISO = np.zeros(numFiles)
...
for f, filename in enumerate(npzFiles_iso):
    ...
    # Make an array with all summary stats
    Percent_Moving_NS_BINS_ISO[f] = Percent_Moving_NS_BINS
    Percent_Moving_S_BINS_ISO[f] = Percent_Moving_S_BINS
...
# Allocate space for summary data
Percent_Moving_NS_BINS_DRUG = np.zeros(numFiles)
Percent_Moving_S_BINS_DRUG = np.zeros(numFiles)
...
for f, filename in enumerate(npzFiles_drug):
    ...
    # Make an array with all summary stats
    Percent_Moving_NS_BINS_DRUG[f] = Percent_Moving_NS_BINS
    Percent_Moving_S_BINS_DRUG[f] = Percent_Moving_S_BINS
...
m = np.nanmean(Percent_Moving_S_BINS_CTL, 0)
std = np.nanstd(Percent_Moving_S_BINS_CTL, 0)
valid = (np.logical_not(np.isnan(Percent_Moving_S_BINS_CTL)))
n = np.sum(valid, 0)
print(n)
se = std/np.sqrt(n-1)
...
m = np.nanmean(Percent_Moving_S_BINS_ISO, 0)
std = np.nanstd(Percent_Moving_S_BINS_ISO, 0)
valid = (np.logical_not(np.isnan(Percent_Moving_S_BINS_ISO)))
n = np.sum(valid, 0)
print(n)
se = std/np.sqrt(n-1)
...
m = np.nanmean(Percent_Moving_S_BINS_DRUG, 0)
std = np.nanstd(Percent_Moving_S_BINS_DRUG, 0)
valid = (np.logical_not(np.isnan(Percent_Moving_S_BINS_DRUG)))
n = np.sum(valid, 0)
print(n)
se = std/np.sqrt(n-1)
...
plt.tight_layout() 
filename = figureFolder +'/Figure_4B_mobility.png'
plt.savefig(filename, dpi=600)
filename = figureFolder +'/Figure_4B_mobility.eps'
plt.savefig(filename, dpi=600)
plt.close('all')
...
#-----------------------------------------------------------------------------
report_path = figureFolder + '/report_Figure4B_mobility.txt'
report_file = open(report_path, 'w')

# Controls vs Short Isolation Drugged
for i in range(15):
    S1 = Percent_Moving_S_BINS_CTL[:,i]
    valid = (np.logical_not(np.isnan(S1)))
    S1 = S1[valid]

    S2 = Percent_Moving_S_BINS_DRUG[:,i]
...
for i in range(15):
    S1 = Percent_Moving_S_BINS_ISO[:,i]
    valid = (np.logical_not(np.isnan(S1)))
    S1 = S1[valid]
