...
def fill_bouts(bouts, FPS):
    ...
    if (FPS == 100):
        ...
        moving_frames = moving_frames[:108000]
...
# LOAD CONTROLS
PTM_NS_BINS_CTL, PTM_S_BINS_CTL = load_and_process_npz(npzFiles_ctl)

# LOAD ISO
PTM_NS_BINS_ISO, PTM_S_BINS_ISO = load_and_process_npz(npzFiles_iso)

# LOAD DRUGGED
PTM_NS_BINS_DRUG, PTM_S_BINS_DRUG = load_and_process_npz(npzFiles_drug)
...
m = np.nanmean(PTM_S_BINS_CTL, 0)
std = np.nanstd(PTM_S_BINS_CTL, 0)
valid = (np.logical_not(np.isnan(PTM_S_BINS_CTL)))
n = np.sum(valid, 0)
se = std/np.sqrt(n-1)
...
m = np.nanmean(PTM_S_BINS_ISO, 0)
std = np.nanstd(PTM_S_BINS_ISO, 0)
valid = (np.logical_not(np.isnan(PTM_S_BINS_ISO)))
n = np.sum(valid, 0)
se = std/np.sqrt(n-1)
...
m = np.nanmean(PTM_S_BINS_DRUG, 0)
std = np.nanstd(PTM_S_BINS_DRUG, 0)
valid = (np.logical_not(np.isnan(PTM_S_BINS_DRUG)))
n = np.sum(valid, 0)
se = std/np.sqrt(n-1)
...
filename = figureFolder +'/Figure_4B_ptm.eps'
plt.savefig(filename, dpi=600)
plt.close('all')
...
#-----------------------------------------------------------------------------
report_path = figureFolder + '/report_Figure4B_ptm.txt'
report_file = open(report_path, 'w')

# Controls vs Short Isolation Drugged
for i in range(15):
    bin_label = str(i)
    do_stats(PTM_S_BINS_CTL[:,i], bin_label + " Controls S", PTM_S_BINS_ISO[:,i], "Partial Isolation S", report_file)
