...
# 1. Compute Viewing Preference Index
def computeVPI(xPositions, yPositions, testROI, stimROI, FPS=100):
...
# 2. Compute Social Preference Index
...
# 3. Measure ditance traveled during experiment (in mm)
...
# 4. Compute normalized arena coordinates
...
def computeTPI(xPositions, yPositions, testROI, stimROI):
   ...
   # Check X threshold 
    if socialLeft:
       ...
       newthreshold= socialPositionThreshold_X+ (2*(testROI[2]/4))
       AllSocialFrames_X_TF= xPositions>newthreshold   # Opposite True/False array
